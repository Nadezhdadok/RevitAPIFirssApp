...
namespace RevitAPITraining341
{
    ...
    public class Main : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            foreach (var selectedElement in selectedElementRefList)
            {
                ...
                if (lengthParameter.StorageType == StorageType.Double)
                {
                    ...
                    {
                        ts.Start();
                        Parameter commentParameter = getElement.LookupParameter("Наименование");
                        commentParameter.Set("Труба", $"{lengthStock} mm") ;
                        ts.Commit();
                    }
                }
            }
        ...
        private void CreateSharedParameter(Application application, Document doc, string v1, CategorySet categorySet, BuiltInParameterGroup pG_GEOMETRY, bool v2)
        {
